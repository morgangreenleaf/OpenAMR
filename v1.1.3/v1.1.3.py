...
def zonefinderInitializer():
    ...
    for zonefinderDB_Obj in callzonefinder.fetchall():
        if zonefinderDB_Obj[0] < 1:
            try:
                exec_stateSet.execute(
                    "UPDATE petri_dish SET exec_state = 1 WHERE  sample_id = " + str(
                        zonefinderDB_Obj[3]))
                dbhzonefinder.commit()
            ...
            notificationBtn["text"] = "PROCCESSING COMPLETED TEST "

            old_img = imglocationhome + str(zonefinderDB_Obj[3]) + ".png"
            ...
            def executeTestCompleted():
                ...
                global discs
                try:
                    if PhotoImage(file=old_img):
                        newImageFile, arrayDistances, discs = zonefinder(old_img)
                except:
                    print("Image not found")
                    pass

            zonefinderInitializerThread = threading.Thread(
                target=executeTestCompleted,
            ...
            def waitForThreadtoComplete():
                ...
                if zonefinderInitializerThread.isAlive():
                    ...
                    notificationBtn["text"] = "PROCCESSED POCKET " + str(zonefinderDB_Obj[2]) + " TEST"
                    next_color = "Black"
                    notificationBtn.config(foreground=next_color)

                    discsObject = dbhzonefinder.cursor()
                    discsObj = dbhzonefinder.cursor()

                    try:
                        discsObj.execute("DELETE FROM zones WHERE sample_id=" + str(zonefinderDB_Obj[3]))
                        dbhzonefinder.commit()
                    except pymysql.Error as e:
                        dbhzonefinder.rollback()
                        print(e)
                    ...
                    for a1 in discs:
                        ...
                        try:
                            discsObject.execute(
                                "INSERT INTO zones (disc,sample_id) VALUES ('" + str(
                                    discs[a1]) + "'," + str(
                                    zonefinderDB_Obj[3]) + ")")
                            dbhzonefinder.commit()
                    ...
                    distanceToarrary = None
                    distanceDict = {}
                    insertIntoDiameter = dbhzonefinder.cursor()
                    checkAbx = dbhzonefinder.cursor()
                    checkAbx.execute(
                        "SELECT disc_id  FROM discs WHERE sample_id =" + str(
                            zonefinderDB_Obj[3]) + "")

                    for arrayKey, arrayVal in enumerate(list_distance):
                        distanceToarrary = math.floor(arrayVal)
                        arrayKey += 1
                        distanceDict[arrayKey] = distanceToarrary

                    for (distancekey, distanceDictval), disc_num in zip(distanceDict.items(),
                                                                        checkAbx.fetchall()):
                        try:
                            insertIntoDiameter.execute(
                                "UPDATE discs set diameter=" + str(
                                    distanceDictval) + " WHERE sample_id=" + str(
                                    zonefinderDB_Obj[3]) + " and disc_id =" + str(
                                    disc_num[0]))
                            dbhzonefinder.commit()
                        except pymysql.Error:
                            print(pymysql.Error)
                    ...
                    imgcompleted = imglocationhome + "zonesfound" + str(
                        zonefinderDB_Obj[3]) + ".png"
                    try:
                        discsObject.execute(
                        ...
                        dbhzonefinder.commit()
                        discsObject.execute(
                            "UPDATE pocket_state set state = 1 WHERE pocket_id = " + str(
                                zonefinderDB_Obj[5]))
                        dbhzonefinder.commit()
                        discsObject.execute(
                            "UPDATE petri_dish set state = 3 WHERE sample_id = " + str(
                                zonefinderDB_Obj[3]))
                        dbhzonefinder.commit()
                    except pymysql.Error:
                        print(pymysql.Error)
            ...
            dbhzonefinder.commit()
    threading.Timer(5, zonefinderInitializer).start()
        ...
        if patient_id.get() == '':
            ...
            def petriDish_selection():
                if isolate_name.get() == '':
                    ...
                    resetTextThreadi = threading.Thread(target=resetTexti)
                    resetTextThreadi.start()
                    ...
                    def petriDishSelected():
                        dbh.commit()
                        pocketcountPlus = dbh.cursor()
                        pocketcountPlus.execute("SELECT COUNT(*) FROM pocket_state WHERE state=1")
                        for p_count in pocketcountPlus.fetchall():
                            pdishNumber.set(p_count[0])

                        if pdishNumber.get() == 0:
                            notificationBtn12["text"] = "ALL POCKETS ARE IN USE NOW"
                        else:
                            def stepper_camera():
                                dbh.commit()
                                ...
                                for p_no in pocket_number.fetchall():
                                    pdish_n.set(p_no[0])

                                notificationBtn12["text"] = "PROCESSING PLEASE WAIT"
                                p_dishLabel["state"] = "disabled"
                                p_dishLabel["image"] = proceedImg
                                p_dishLabel["command"] = moveMotorToCameraPos
                                en_samplewindow.destroy()
                                # move motor and start Camera methods come here
                                time.sleep(1)
                                notificationBtn12["text"] = "PROCESSING COMPLED"
                                p_dishLabel["state"] = "enabled"

                                # p_Label.pack(pady=10, padx=10, side=LEFT)

                            stepper_cameraThread = threading.Thread(target=stepper_camera)
                            stepper_cameraThread.start()

                    def moveMotorToCameraPos():
                        p_dishLabel["state"] = "disabled"
                        # p_dishTerminate["state"] = "disabled"
                        ...
                        def locadiscsRun():
                            ...
                            try:
                                notificationBtn12["text"] = "DETECTING ANTIBIOTICS"
                            except:
                                pass
                        ...
                        def waitTimeRunner():
                            ...
                            if locadiscsThread.isAlive():
                                ...
                                try:
                                    notificationBtn12["text"] = "PROCESSING COMPLETED"
                        ...
                        notificationBtn12["text"] = "PROCESSING IMAGE PLEASE WAIT"

                        def change_color():
                            try:
                                current_color = notificationBtn12.cget("foreground")
                                next_color = "white" if current_color == "black" else "black"
                                notificationBtn12.config(foreground=next_color)
                        ...
                        def discidentifierMethod():
                            #  global  notificationLabelFinish
                            ...
                            global petri_dish_view, imgpath, coordinates, images, masks, abx_result_dictionary
                            ...
                            def saveTestData():
                                ...
                                def processingData():
                                    notificationLabelFinish["text"] = "PROCCESSING PLEASE WAIT"
                                    ...
                                    try:
                                        ...
                                        insert_into_petri_dishTable.execute(
                                            "INSERT INTO petri_dish (dish_id,sample_id,endTime) VALUES (" + pdish_n.get() + ",(SELECT sample_id FROM samples WHERE accession_number='" + patient_id.get() + "' ),'" + time_stample.get() + "')")
                                        update_pocket_stateTable.execute(
                                            "UPDATE pocket_state SET state=0 WHERE  pocket_id =" + pdish_n.get() + "")
                                        countAbx = 0
                                        for _ in abx_result_dictionary:
                                            ...
                                            add_abx_todiscsTable.execute(
                                                ...
                                                abx_result_dictionary[
                                                    countAbx] + "')," + pdish_n.get() + ",(SELECT sample_id FROM samples WHERE accession_number='" + patient_id.get() + "' ))")
                                        dbh.commit()
                                ...
                                def waitForProgressToComplete():
                                    ...
                                    if processingDataThread.isAlive():
                                        ...
                                        print(flag)
                                        if flag:
                                            finishBtn.pack_forget()
                                            cancelBtn2["image"] = okImg
                                            notificationLabelFinish["text"] = "TEST IN PROGRESS"
                                            ...
                                            finishBtn.pack_forget()
                                            notificationLabelFinish["text"] = "PROCESS EXECUTION FAILED"
                                            cancelBtn2["image"] = okImg
                                            base.after_cancel(waitForLoop)
                            ...
                            def confirm_button():
                                global notificationLabelFinish
                                global rootConfirm, pocket_root
                                pocket_root.destroy()

                                rootConfirm = Toplevel(base, background="#FFF")
                                conformFrame = cal.Frame(rootConfirm, relief=SOLID, padding=20)
                                ...
                                cal.Label(conformFrame, text="Isolate Name : " + isolate_name.get()).pack(
                                    anchor="w")
                                cal.Label(conformFrame, text="ANTIBIOTICS").pack(pady=1, anchor="w")
                                ...
                                optFininshFrame = cal.Frame(conformFrame)
                                finishBtn = cal.Button(optFininshFrame, image=okImg, width=30, padding=20,
                                                       command=saveTestData)
                                cancelBtn2 = cal.Button(optFininshFrame, image=cancelImg, width=30, padding=20,
                                                        command=rootConfirm.destroy)
                                cancelBtn2.pack(pady=10, padx=10, side=LEFT, ipadx=35)
                                finishBtn.pack(pady=10, side=LEFT, padx=10, ipadx=35)

                                optFininshFrame.pack()

                                finishFrame = cal.LabelFrame(conformFrame, relief=SOLID, padding=20,
                                                             text="Notifications")

                                notificationLabelFinish = Label(finishFrame, width=30, text="NO PROCESS RUNNING",
                                                                font=("SEGOE UI", 15))
                                notificationLabelFinish["background"] = "#fff"
                                notificationLabelFinish.pack(pady=5, side=LEFT)
                    ...
                    p_dishTerminate = cal.Button(petri_dish_frameOpt, image=backImg, padding=20,
                                                 command=petri_dish_view.destroy)
                    p_dishTerminate.pack(pady=10, padx=10, ipadx=20, side=LEFT)
                    p_dishLabel = cal.Button(petri_dish_frameOpt, text="1 PETRI DISH", padding=20,
                                             command=petriDishSelected)
                    p_dishLabel.pack(pady=10, padx=10, side=LEFT, ipadx=20, )

                    # =========================================================================================

                    petri_dish_frameOpt.pack()

                    label_frame11 = cal.LabelFrame(petri_dish_frame, relief=SOLID, padding=20, text="Notifications")
                    notificationBtn12 = Label(label_frame11, width=28, text="NO PROCESS RUNNING", font=("SEGOE UI", 15))
                    notificationBtn12["background"] = "#fff"
                    notificationBtn12.pack(pady=5, side=LEFT)
                    label_frame11.pack()

                    cal.Button(petri_dish_frame, image=proceedImg, width=29, padding=15,
                               command=petri_dish_view)
            ...
            titleBarIsolate = cal.Label(en_samplewindow, text="CHOOSE ISOLATE  | ID [" + patient_id.get() + "]")
            titleBarIsolate.pack(pady=10)
                ...
                def singleTestProgressing(val):
                    ...
                    def editResult(a):
                        ...
                        def circle_adjust(disc, plus_minus):
                            ...
                            list_distances = zone_distances.copy()
                            ...
                            for listKey, listVal in enumerate(list_distances):
                                distanceDict_val[listKey] = round(listVal)

                            for (dictKey, dictVal), disc_id in zip(distanceDict_val.items(), checkAbx.fetchall()):
                                try:
                                    insertIntoDiameter.execute(
                                        "UPDATE discs set diameter=" + str(
                                            dictVal) + " WHERE sample_id=" + str(
                                            sample_id.get()) + " and disc_id =" + str(disc_id[0]) + "")
                                    dbh.commit()
                                except pymysql.Error as e:
                                    print(e)
                                    dbh.rollback()
...
if __name__ == '__main__':
    ...
    zonefinderInitializerQ = "SELECT timestampdiff(hour,now(),pd.endTime) as durationLeft,sa.accession_number,pd.dish_id, pd.sample_id, pd.dish_id , " + str(
        " ") + "ps.pocket_id FROM samples sa INNER JOIN petri_dish pd inner JOIN pocket_state ps on pd.dish_id=ps.pocket_id and " + str(
        " ") + "sa.sample_id=pd.sample_id where pd.state =1 and pd.exec_state = 0  ORDER BY durationLeft ASC LIMIT 1 "
    ...
    okImg = PhotoImage(file="assets/ico/ok.png")
    cancelImg = PhotoImage(file="assets/ico/cancel.png")
    proceedImg = PhotoImage(file="assets/ico/proceed.png")

    incompletedExec = dbh.cursor()
    try:
        incompletedExec.execute("UPDATE petri_dish SET  exec_state = 0 WHERE exec_state=1 AND state= 1")
        dbh.commit()
    except pymysql.Error as e:
        print(e)
        dbh.rollback()
    ...
    timeConfirm = dbh.cursor()
    timeConfirm.execute("SELECT timestampdiff(day,('2018-09-03 21:39:10'),now()) as a")
    # timeConfirm.execute("SELECT timestampdiff(second,now(),(SELECT tested_on FROM petri_dish ORDER BY `petri_dish`.`tested_on` DESC LIMIT 1)) as timeTo")
    for _ in timeConfirm.fetchall():
        pass
