...
def stored_tests():
    ...
    def cmptest():
        ...
        for d in getCursor.fetchall():
            ...
            cal.Button(cFrame, image=okImg, padding=10, command=topLevelWin.destroy).pack(side=RIGHT, pady=10)
            cal.Button(cFrame, image=photoImg, padding=10, command=edTest).pack(side=LEFT, pady=10)
    ...
    cmpListbox.bind("<Return>", lambda e, box=cmpListbox: cmptest())
    cmpListbox.bind("<Double-Button-1>", lambda e, box=cmpListbox: cmptest())
    cmpListbox.pack()
    cal.Button(abxFrame, image=cancelImg, padding=10, command=topLevelWin.destroy).pack(side=LEFT, pady=10)


    cal.Button(abxFrame, image=okImg, padding=10, command=cmptest).pack(side=RIGHT, pady=10)
    anEntry.pack()
    abxFrame.pack()
...
def start_test():
    ...
    def proceedtophoto():
        ...
        # noinspection PyShadowingNames
        def discabxConfirm():
            global imgpath, images, antiobioticsDis, imagedis, imageScale, topLevelWin, isolatename
            topLevelWin.destroy()
            topLevelWin = Toplevel(base)
            topLevelWin.iconbitmap(icoloc)
            ...
            for i in antiobioticsDis:
                ...
                if i > 3:
                    ...
                    disclabeldict[i] = cal.Label(discabxR, text="Disc " + str(i) + "." + antiobioticsDis[i])
                    disclabeldict[i].pack(anchor='w', pady=0)
                    disclabeldict[i].bind("<Button-1>", lambda eventhandle, akey=i, aname=disclabeldict[i]: abToChange(aname, akey))
            ...
            def testData():
                ...
                for d in getCursor.fetchall():
                    ...
                    generalOpt[i + 5]["text"] = "Verifying image to process"
                    time.sleep(2)
                    ...
                    def waitFor():
                        ...
                        if not myflag:
                            ...
                            generalOpt[i + 5]["text"] = "Process completed"
                            time.sleep(3)
                            iflag = False
                    waitFor()
                    ...
                    def singleTestProgressing():
                        ...
                        for Id in getCursor.fetchall():
                            cal.Label(topLevelWin, text="unique code[" + str(Id[0]) + "] bacteria [" + str(Id[1][:10] + ".. " if len(Id[1]) > 10 else Id[1]) + "]").pack(pady=10)
                        generalOpt = cal.Frame(cFrame, padding=5, relief=SOLID)
                        cal.Label(generalOpt, text="Antibiotics", width=15).pack(side="left", padx=3, anchor="w")
                        cal.Label(generalOpt, text="Dose", width=4).pack(side="left", padx=3, anchor="w")
                        cal.Label(generalOpt, text="MM", width=4).pack(side="left", padx=3, anchor="w")
                        ...
                        def edTest():
                                ...
                                def circle_adjust(dsc, cond):
                                    ...
                                    for (_, di), disc_id in zip(dist.items(), getCursor.fetchall()):
                                        try:
                                            dbh.cursor().execute("UPDATE discs set diameter=" + str(di) + " WHERE sample_id='" + str(d[0]) +
                                    ...
                                    getCursor.execute("SELECT imagelocation FROM images WHERE state=3 AND sample_id= '" + str(d[0]) + "'")
                                ...
                                generals[g + 3].bind("<Button-1>", lambda event: circle_adjust(generals[g + 2].get(), True))
                                ...
                                generals[g + 4].bind("<Button-1>", lambda event: circle_adjust(generals[g + 2].get(), False))
                                generals[g + 4].pack(side=LEFT, padx=2)
                                bFrame.pack()
                                cal.Button(eFrame, text="Confirm", padding=3, width=10, command=topLevelWin.destroy).pack(pady=3)
                                eFrame.pack()
            ...
            def datacollectionDone():
                    ...
                    global topLevelWin, isolatename, antiobioticsDis, imagedis, imageScale, uniquecode, generalOpt
                    topLevelWin.destroy()
                    # noinspection PyShadowingNames
                    i = 0
                    topLevelWin = Toplevel(base)
            ...
            def anproceed():
                global generalOpt
                generalOpt = False
                for abKey in antiobioticsDis:
                    if antiobioticsDis[abKey] == "No match found":
                        generalOpt = True
...
if __name__ == '__main__':
    ...
    # "wm attributes window ?-alpha ?double?? ?-transparentcolor ?color?? ?-disabled ?bool?? ?-fullscreen ?bool??
    # ?-toolwindow ?bool?? ?-topmost ?bool??"
